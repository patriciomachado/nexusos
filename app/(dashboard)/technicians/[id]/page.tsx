                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 p-8 rounded-2xl bg-card/60 border border-border relative overflow-hidden group">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row gap-10 items-start">
                            <div className="w-32 h-32 rounded-2xl bg-muted/20 border border-border flex items-center justify-center text-5xl font-black text-orange-400 relative">
                            ...
                            <div className="flex-1 space-y-6">
                                <div>
                                    <div className="flex items-center gap-4 mb-2">
                                        ...
                                        <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-semibold border border-emerald-500/20">Ativo</span>
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {technician.specialties?.map((s: string) => (
                                        <span key={s} className="px-4 py-2 rounded-xl bg-muted/5 border border-border text-xs font-bold text-muted-foreground">
                                            {s}
                                        </span>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-4">
                        <div className="p-6 rounded-2xl bg-muted/[0.02] border border-border group hover:border-indigo-500/20 transition-all">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-xs font-semibold text-foreground/20">Visão Geral</span>
                            </div>
                            <div className="space-y-1">
                                <p className="text-3xl font-black text-foreground tracking-tighter">{completedOS + inProgressOS}</p>
                                <p className="text-xs font-bold text-foreground/30">Total de Atendimentos</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-5 rounded-2xl bg-muted/[0.02] border border-border">
                                <p className="text-2xl font-black text-emerald-400 tracking-tighter">{completedOS}</p>
                                <p className="text-xs font-bold text-foreground/20">Concluídas</p>
                            </div>
                            <div className="p-5 rounded-2xl bg-muted/[0.02] border border-border">
                                <p className="text-2xl font-black text-orange-400 tracking-tighter">{inProgressOS}</p>
                                <p className="text-xs font-bold text-foreground/20">Em Aberto</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {serviceOrders && serviceOrders.length > 0 ? serviceOrders.map((os) => {
                                    ...
                                    <div className="relative z-10 flex items-center justify-between">
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3">
                                                <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${statusClasses}`}>
                                                    {OS_STATUS_LABELS[os.status]}
                                                </span>
                                                <span className="text-xs font-mono text-foreground/20">#{os.order_number}</span>
                                            </div>
                            ...
                            <div className="md:col-span-2 p-20 text-center border-2 border-dashed border-border/5 rounded-2xl">
                                <Wrench className="w-12 h-12 text-foreground/5 mx-auto mb-4" />
                                <p className="text-muted-foreground font-bold">Nenhuma atividade registrada</p>
                            </div>
                        )}
                    </div>
                </div>
