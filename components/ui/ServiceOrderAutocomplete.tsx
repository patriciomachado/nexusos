...
import { Search, ClipboardList, Check, ChevronsUpDown } from 'lucide-react'
...
export default function ServiceOrderAutocomplete({ serviceOrders, selectedId, onSelect, error, placeholder, label }: Props) {
    const [isOpen, setIsOpen] = useState(false)
    const [query, setQuery] = useState('')
    const containerRef = useRef<HTMLDivElement>(null)

    const selectedSO = serviceOrders.find(so => so.id === selectedId)

    const filteredSO = query === ''
        ? serviceOrders.slice(0, 10)
        : serviceOrders.filter(so =>
            so.title.toLowerCase().includes(query.toLowerCase()) ||
            so.order_number.toString().includes(query)
        ).slice(0, 10)
        ...
        <div ref={containerRef} className="relative w-full">
            ...
            >
                <div className="flex items-center gap-3">
                    <ClipboardList className={cn("w-4 h-4 transition-colors", selectedSO ? "text-primary" : "text-muted-foreground/30")} />
                    <span className={cn("block truncate", !selectedSO && "text-muted-foreground/50")}>
                        {selectedSO ? `#${selectedSO.order_number} - ${selectedSO.title}` : (placeholder || "Vincular OS...")}
                    </span>
                </div>
                        ...
                        {filteredSO.length > 0 ? (
                            filteredSO.map((so) => (
                                <li key={so.id}>
                                    ...
                                    >
                                        <div className={cn(
                                            "w-10 h-7 rounded-lg flex items-center justify-center text-[9px] font-black uppercase transition-all",
                                            so.id === selectedId
                                        ...
                                        <span className="flex-1 text-left truncate font-bold">{so.title}</span>
                                        {so.id === selectedId && (
                            ...
                            <li className="px-4 py-8 text-center">
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/20">Nenhuma OS encontrada</p>
                            </li>
                        )}
                    </ul>
                </div>
