...
export default function ProductCatalog() {
        ...
        <div className="flex flex-col h-full space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 shrink-0">
                <div 
                    ...
                    className="flex items-center gap-2 glass-premium bg-foreground/[0.03] p-1.5 rounded-2xl border border-border/60 overflow-x-auto no-scrollbar cursor-grab active:cursor-grabbing select-none"
                >
                    <button
 onClick={() => setActiveCategory('all')}
 className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-semibold transition-all shrink-0 ${activeCategory === 'all'
 ...
 className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-semibold transition-all shrink-0 ${activeCategory === 'servicos'
 ? 'bg-primary text-primary-foreground scale-105'
 : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-semibold transition-all shrink-0 ${activeCategory === cat.id
 ? 'bg-primary text-primary-foreground scale-105'
 : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
 }`}
 >
            ...
            <div className="flex-1">
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-64 space-y-4">
                        ...
                        <p className="text-xs font-semibold text-muted-foreground">Sincronizando Catálogo...</p>
                    </div>
                ) : products.length > 0 ? (
                    <div className="grid grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 lg:gap-6">
                        {products.map((product) => (
                            <ProductCard key={product.id} product={product} />
                        ))}
                    </div>
                ) : (
                    <div className="flex flex-col items-center justify-center p-24 text-center space-y-6 border-2 border-dashed border-border rounded-2xl opacity-40">
